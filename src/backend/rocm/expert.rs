...
impl RocmContext {
    ...
    fn prefill_resident_routed_experts_impl(
        ...
        if matches!(experts.archive, RocmExpertArchive::Gguf(_)) {
            ...
            let metas = experts.gguf_grouped_metas(self.device_id, layer, spec.num_experts)?;
            let metas = ops::hip::resident_gguf_grouped_metas(self.device_id, &metas).map_err(compute_error)?;
...
impl RocmPrefillExperts {
    ...
    pub fn preload_layer(&mut self, backend: &RocmContext, layer: usize, expert_count: usize) -> Result<(), BackendError> {
        ...
        self.load_batch(backend, layer, &expert_ids).map(|_| ())
    }
    ...
    fn gguf_grouped_metas(&self, device: i32, layer: usize, expert_count: usize) -> Result<Vec<ops::hip::GgufGroupedExpertMeta>, BackendError> {
        let meta = |weight: &RocmWeight| -> Result<(u32, u64), BackendError> {
            match weight.quantized() {
                Some(RocmQuantizedWeight::GgufPacked { codes, tensor_type, .. }) => Ok((*tensor_type, codes.device_pointer() as u64)),
                _ => Err(BackendError::ExpertLoad(format!("GGUF grouped L{layer} 权重不是 GGUF 打包形态"))),
            }
        };
        (0..expert_count)
            .map(|expert| {
                let resident = self.resident.get(&(device, layer, expert)).ok_or_else(|| BackendError::ExpertLoad(format!("GGUF grouped L{layer} E{expert} 未常驻")))?;
                let (gate_type, gate) = meta(&resident.gate)?;
                let (up_type, up) = meta(&resident.up)?;
                let (down_type, down) = meta(&resident.down)?;
