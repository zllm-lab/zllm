...
// 注意：本文件与 fp16_tile_matmul.cpp 导出同名 extern "C" 符号
// fp16_tile_matmul，二者是同一算子的两个候选实现，编译脚本每次只拷贝
// 其中一份（compile_quant_resident_decode_kirin9010.sh 使用本文件，
// 配 q6/q8_decode_tiles.cpp），禁止同时编译。
// 本实现的 L0A 输入 tile 按 k-major 打包：(k_tile * m_tiles + m_tile)，
// block 为 128x128x64。
// 输入已经由 AIV 排成连续的 16x16 Cube tile。AIC 只执行 GM->L0、Mmad
// 与 Fixpipe，不在 Cube 核中做任何标量 GM 访问。
