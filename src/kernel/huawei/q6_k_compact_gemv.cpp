...
// Q6_K 的 4-bit low / 2-bit high 仍保持 packed；每块只把 d*scale 预计算
// 为 16 个 FP16。9010 在 UB 中向量解包，避免 320-byte 全展开布局越过
// 单图 64 MiB Const 访问窗口。
class KernelQ6KCompactGemv {
public:
    __aicore__ inline KernelQ6KCompactGemv() {}
    ...
    __aicore__ inline void Process()
    {
        ...
        // columns 同时被 256(q6_block) 与 9*256=2304(block_batch*q6_block) 整除、
        ...
        for (uint32_t block_lane = 0; block_lane < block_batch;
            ...
            for (uint32_t group = 0; group < groups_per_block; ++group) {
                ...
                const uint32_t half_block = group / 4;
                const uint32_t quarter = group % 4;
                const uint32_t low_source =
                    half_block * 64 + (quarter & 1) * 32;
                const uint32_t high_source = half_block * 32;
                const uint32_t shift = quarter * 2;
                AscendC::CreateVecIndex(
                    code_indices[element].ReinterpretCast<int16_t>(),
                    static_cast<int16_t>(
                        block_offset + low_offset + low_source),
                    group_elements);
                AscendC::CreateVecIndex(
                    high_indices[element].ReinterpretCast<int16_t>(),
                    static_cast<int16_t>(
                        block_offset + high_offset + high_source),
                    group_elements);
                AscendC::Duplicate(
                    scale_indices[element],
                    static_cast<uint16_t>(block_offset + group * 4),
                    group_elements / 2);
                AscendC::Duplicate(
                    scale_indices[element + group_elements / 2],
                    static_cast<uint16_t>(block_offset + group * 4 + 2),
                    group_elements / 2);
                const bool high_nibble = quarter >= 2;
                AscendC::Duplicate(
                    low_mask[element],
                ...
                AscendC::Duplicate(
                    high_mask[element],
                    static_cast<uint8_t>(0x03u << shift),
                    group_elements);
                AscendC::Duplicate(
                    high_factor[element],
                    static_cast<half>(
...
private:
    ...
    static constexpr uint32_t compact_bytes = 224;
    static constexpr uint32_t low_offset = 32;
    static constexpr uint32_t high_offset = 160;
    static constexpr uint32_t output_tile = 16;
    // 18 blocks 会把 packed 解码所需的 mask/index UB 推到约 138 KiB，
    // Kirin 9010 真机直接报 HWTS 0x2；9 blocks 约 83 KiB。
    static constexpr uint32_t block_batch = 9;
