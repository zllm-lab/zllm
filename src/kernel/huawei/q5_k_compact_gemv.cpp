...
// Q5_K 的 low nibbles / high bits 保持 packed；每块只预计算 8 个 d*scale
// 与 8 个 dmin*min。整层 Const 因而留在 9010 的 64 MiB 访问窗口内。
class KernelQ5KCompactGemv {
public:
    __aicore__ inline KernelQ5KCompactGemv() {}
                                ...
                                uint32_t batch)
    {
        ...
        row_half_elements_ =
            blocks_per_row_ * compact_bytes / sizeof(half);
    ...
    __aicore__ inline void Process()
    {
        ...
        // columns 同时被 256(q5_block) 与 block_batch*256 整除(block_batch 由 ZLLM_Q5_K_COMPACT_BLOCK_BATCH 宏决定,默认 5、down 构建用 9)、
...
#ifdef ZLLM_Q5_K_COMPACT_STREAM_ROWS
        ...
        pipe.InitBuffer(byte_buffer, 6 * tile_elements);
        pipe.InitBuffer(half_buffer, 5 * tile_elements * sizeof(half));
        ...
        auto quant = bytes[0 * tile_elements];
        auto high = bytes[1 * tile_elements];
        auto temporary = bytes[2 * tile_elements];
        auto low_mask = bytes[3 * tile_elements];
        auto high_nibble_mask = bytes[4 * tile_elements];
        auto high_mask = bytes[5 * tile_elements];

        auto halves = half_buffer.Get<half>();
        auto values = halves[0 * tile_elements];
        auto high_values = halves[1 * tile_elements];
        auto high_factor = halves[2 * tile_elements];
        auto scales = halves[3 * tile_elements];
        auto biases = halves[4 * tile_elements];
        ...
        auto high_indices = index[1 * tile_elements];
        auto scale_indices = index[2 * tile_elements];
        auto bias_indices = index[3 * tile_elements];
        auto result_indices =
        ...
        for (uint32_t block_lane = 0; block_lane < block_batch;
             ++block_lane) {
            const uint32_t block_offset = block_lane * compact_bytes;
            ...
            for (uint32_t group = 0; group < groups_per_block; ++group) {
                ...
                AscendC::CreateVecIndex(
                    ...
                    static_cast<int16_t>(
                        block_offset + low_offset +
                        (group / 2) * group_elements),
                    group_elements);
                AscendC::CreateVecIndex(
                    high_indices[element].ReinterpretCast<int16_t>(),
                    static_cast<int16_t>(block_offset + high_offset),
                    group_elements);
                AscendC::Duplicate(
                ...
                AscendC::Duplicate(
                    low_mask[element],
                    static_cast<uint8_t>(high_nibble ? 0 : 0x0f),
                    group_elements);
                AscendC::Duplicate(
                    high_nibble_mask[element],
                    static_cast<uint8_t>(high_nibble ? 0x0f : 0),
                    group_elements);
        ...
        for (uint32_t row_tile = core; row_tile < row_tiles;
...
#ifndef ZLLM_Q5_K_COMPACT_STREAM_ROWS
            ...
            for (uint32_t local_row = 0; local_row < output_tile;
                ...
                for (uint32_t block = 0; block < blocks_per_row_;
                     block += block_batch) {
                    const uint32_t base = block * compact_bytes;
                    AscendC::Gather(quant, packed[base], code_indices, 0,
                                    tile_elements);
                    AscendC::ShiftRight<uint8_t, false>(
                        temporary, quant, static_cast<uint8_t>(4),
                        tile_elements);
                    AscendC::And(quant, quant, low_mask, tile_elements);
                    AscendC::And(temporary, temporary, high_nibble_mask,
                                 tile_elements);
                    AscendC::Or(quant, quant, temporary, tile_elements);
                    AscendC::Gather(high, packed[base], high_indices, 0,
                                    tile_elements);
                    AscendC::And(high, high, high_mask, tile_elements);
                    AscendC::Cast(values, quant,
                                  AscendC::RoundMode::CAST_NONE,
                                  tile_elements);
                    AscendC::Cast(high_values, high,
                                  AscendC::RoundMode::CAST_NONE,
                                  tile_elements);
                    AscendC::Mul(high_values, high_values, high_factor,
                                 tile_elements);
                    AscendC::Add(values, values, high_values, tile_elements);
...
private:
    ...
    static constexpr uint32_t compact_bytes = 192;
    static constexpr uint32_t bias_offset = 16;
    static constexpr uint32_t high_offset = 32;
    static constexpr uint32_t low_offset = 64;
    static constexpr uint32_t output_tile = 16;
    // 9010 虽报告 122,880-byte UB，Q5 的 10-block 工作集仍会触发
    // HWTS 0x2；5-block 是连续 FFN 图已经真机验证的安全上限。
#ifndef ZLLM_Q5_K_COMPACT_BLOCK_BATCH
#define ZLLM_Q5_K_COMPACT_BLOCK_BATCH 5
#endif
    static constexpr uint32_t block_batch =
        ZLLM_Q5_K_COMPACT_BLOCK_BATCH;
