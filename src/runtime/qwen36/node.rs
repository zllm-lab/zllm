...
#[cfg(target_os = "macos")]
use crate::{
    config::{NodeMetalBackendConfig, Qwen36NodeModelConfig},
...
#[cfg(target_os = "macos")]
pub async fn run(model: Qwen36NodeModelConfig, backend: NodeMetalBackendConfig, config: crate::server::node::NodeConfig) -> Result<(), DynError> {
    ...
    let factory = Box::new(move |runtime, compute_steps| {
        Qwen36Engine::load(&model.weights_directory, model.max_sequence_length, model.variant, model.execution, backend.replay, model.lm_head_quantization, cache_directory, persist_kv_cache, resident_cache_entries, runtime, compute_steps)
...
#[cfg(not(target_os = "macos"))]
pub async fn run(_model: Qwen36NodeModelConfig, _backend: crate::config::NodeMetalBackendConfig, _config: crate::server::node::NodeConfig) -> Result<(), DynError> {
