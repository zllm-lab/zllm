...
pub fn run(
    ...
    let audio_samples = audio_paths.iter().map(read_wav_16khz).collect::<Result<Vec<_>, _>>()?;
...
#[cfg(target_os = "macos")]
fn read_wav_16khz(path: &PathBuf) -> Result<Vec<f32>, String> {
    ...
    let mono = samples.chunks_exact(channels).map(|frame| frame.iter().sum::<f32>() / channels as f32).collect::<Vec<_>>();
    Ok(resample_to_16khz(&mono, sample_rate))
}

#[cfg(target_os = "macos")]
fn resample_to_16khz(samples: &[f32], sample_rate: usize) -> Vec<f32> {
