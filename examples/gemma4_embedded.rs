...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config = arguments.next().ok_or("用法: cargo run --example gemma4_embedded -- <embedded.yaml> <prompt> [max_tokens]")?;
    ...
    if std::env::var_os("ZLLM_EMBEDDED_WARMUP").is_some() {
        ...
        eprintln!("warmup={} tokens wall={:.3}s", result.completion_tokens, warmup_started.elapsed().as_secs_f64());
    }
    let cancellation = engine.cancellation();
...
fn main() {
    eprintln!("gemma4_embedded 仅支持 macOS Metal");
}
